...
<div class="p-0 p-sm-4">
    <div class="border bg-white p-0 p-sm-3">
        <div class="container-fluid">
            <div class="row g-2">
                <div class="col-12">
                    <form action="{{ route('admin.students.index') }}" method="GET" class="row align-items-center justify-content-end mt-3 mt-sm-0 g-2">
                        <input type="hidden" class="form-control shadow-none rounded-0" name="entries" value="{{ request('entries', 5) }}" autocomplete="off">


                        <div class="col-auto">
                            <select class="form-select form-select-sm rounded-0 shadow-none" name="course_id" onchange="this.form.submit()">
                ...
                <div class="col-auto ms-auto">
                    <a href="{{ route('admin.students.create') }}" class="btn btn-sm btn-primary btn-add rounded-0 d-inline-flex align-items-center gap-1">
                ...
                <div class="col-auto">
                    <div class="dropdown">
                        <button type="button" class="btn btn-warning btn-sm rounded-0 shadow-none dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false" data-bs-auto-close="outside">
                            Import
                        </button>
                        <form action="{{ route('admin.students.import') }}" method="POST" class="dropdown-menu p-4 rounded-0" style="width: 300px;" enctype="multipart/form-data">
                            @csrf
                            <div class="mb-3">
                                <label class="form-label">Course</label>
                                <select class="form-select form-select-sm rounded-0 shadow-none" name="course">
                                    @foreach($courses as $course)
                                    <option value="{{ $course->id }}">{{ $course->course }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <label for="student_file" class="btn btn-primary btn-sm rounded-0 shadow-none">Browse Excel</label>
